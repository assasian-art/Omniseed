...
from transformers.initialization import zeros_
...
class Rwkv7CacheLayer(LinearAttentionLayer):
    ...
    # This file ships as Hub remote code, so it has to run on RELEASED transformers,
    # whose `LinearAttentionLayer` carries one recurrent state per layer -- the
    # multi-state form (`number_of_states=`) exists only on current main. Rather than
    # tracking either side's internals, this class owns its own three-slot state
    # machinery outright: the constructor tries the modern base signature and falls
    # back to the released one, then normalises the state attributes to the list form
    # both code paths below assume, and the two methods the model actually exercises
    # (`lazy_initialization`, `update_recurrent_state`) are implemented here in full
    # instead of deferring to a base whose signature has already changed once.

    def __init__(self, number_of_states: int = 3):
        try:
            super().__init__(number_of_states=number_of_states)
        except TypeError:  # released transformers: single-state signature
            super().__init__()
        self.number_of_states = number_of_states
        self.recurrent_states = [None] * number_of_states
        self.is_recurrent_states_initialized = [False] * number_of_states
        self.has_previous_state = [False] * number_of_states
    ...
    def lazy_initialization(self, conv_states=None, recurrent_states=None, state_idx: int = 0, **kwargs) -> None:
        # Recurrent-only: this model has no convolution in front of the mixer, so the
        # conv branch of the upstream implementation is deliberately absent. Device
        # and dtype are recorded here because upstream records them only on the conv
        # branch, and `reorder_cache`'s `beam_idx.to(self.device)` needs them.
...
class Rwkv7Cache(Cache):
    ...
    def write(self, layer_idx: int, att_shift: torch.Tensor, ffn_shift: torch.Tensor, wkv: torch.Tensor) -> None:
        ...
        if torch.is_grad_enabled() and any(state.requires_grad for _, state in states):
            ...
            return
        for slot, state in states:
            self.layers[layer_idx].update_recurrent_state(state, state_idx=slot)
